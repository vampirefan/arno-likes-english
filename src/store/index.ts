...
export const currentDictIdAtom = atomWithStorage('currentDict', 'arno1')
export const currentDictInfoAtom = atom<Dictionary>((get) => {
  const id = get(currentDictIdAtom)
  let dict = idDictionaryMap[id]
  // 如果 dict 不存在，则返回 arno1. Typing 中会检查 DictId 是否存在，如果不存在则会重置为 arno1
  if (!dict) {
    dict = idDictionaryMap.arno1
  }
  return dict
})
